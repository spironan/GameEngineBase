...
namespace oom
{
    ...
    public:
        ...
        template<typename U>
        OOM_EXPLICIT constexpr mat(mat<3, 2, U> const& m);
        ...
        mat<3, 2, T> & operator++ ();
        mat<3, 2, T> & operator-- ();
        mat<3, 2, T> operator++(int);
        mat<3, 2, T> operator--(int);

        // -- Helper Functions --

        static constexpr mat<3, 2, T> zero() { return mat<3, 2, T>{0}; }
        void SetToZero() { *this = zero(); }
        mat<2, 3, T> Transposed() const { return transposed(*this); }
    };
