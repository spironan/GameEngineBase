...
namespace oom
{
    ...
    public:
        ...
        template<typename U>
        OOM_EXPLICIT constexpr mat(mat<2, 3, U> const& m);
        ...
        mat<2, 3, T> & operator++ ();
        mat<2, 3, T> & operator-- ();
        mat<2, 3, T> operator++(int);
        mat<2, 3, T> operator--(int);

        // -- Helper Functions --

        static constexpr mat<2, 3, T> zero() { return mat<2, 3, T>{0}; }
        void SetToZero() { *this = zero(); }
        mat<3, 2, T> Transposed() const { return transposed(*this); }
    };
