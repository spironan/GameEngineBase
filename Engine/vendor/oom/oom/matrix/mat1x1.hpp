...
namespace oom
{
    ...
    struct mat<1, 1, T>
    {
        using col_type          = vec<1, T>;
        using row_type          = vec<1, T>;
        using type              = mat<1, 1, T>;
        using transpose_type    = mat<1, 1, T>;
        using value_type        = T;
    ...
    public:
        ...
        constexpr mat(mat<1, 1, T> const& m);
        ...
        template<typename U>
        OOM_EXPLICIT constexpr mat(mat<1, 1, U> const& m);
        ...
        OOM_EXPLICIT constexpr mat(mat<1, 2, T> const& x);
        OOM_EXPLICIT constexpr mat(mat<2, 1, T> const& x);
